...
function UpdateLecture() {
    ...
    //TODO: Display file names if possible. 
    // TODO: If teacher removes original video but not intended, keep old value in case.

  const location = useLocation();
  const { lecture, subject, classroom } = location.state;
  

  const lectureTitle = useRef(lecture.title);
  const lectureDesc = useRef(lecture.description);
  const lectureLink = useRef(lecture.link);
  const contentTitle = useRef(lecture.contentTitle);
  ...
  useEffect(() => {
      ...
      lectureTitle.current.value = lecture.title;
      lectureDesc.current.value = lecture.description;
      lectureLink.current.value = lecture.link;
      if (lecture.link.startsWith("https://www.youtube.com")) {
          document.querySelector("#headingOne button").click();
      } else {
        document.querySelector("#headingTwo button").click();
      }
      contentTitle.current.value = lecture.contentTitle;
